      ...
      if (err) {
        ...
        cloudinary.uploader.upload(req.file.path, async (err, success) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ msg: "Upload Error :/ Please check your Internet Connection" });
          } else {
            ...
            try {
              const push = await pool.query(
                "Insert into restaurant(name, speciality, location, cuisines, tables, time, available, image) values ($1, $2, $3, $4, $5, $6, $7, $8)",
                data
              );
              if (push.rowCount) {
                return res.json({
                  valid: true,
                  msg: "Restaurant added to the base!",
                  data: restaurant,
                });
              }
            } catch (err) {
  ...
  jwt.verify(req.cookies["Dineat"], process.env.LOB, async (err, payload) => {
    try {
      if (!err) {
        ...
        return res.json({ valid: true, places: [...retrieve.rows], cuisines: cuisines });
      }
    } catch (err) {
      console.log(err);
      return res.status(500).json({ msg: "Database Server Error :/" });
...
const dateFilter = (date) => {
  ...
  return [dateString[1] + "-" + dateString[0] + "-" + dateString[2], tempDate[1].trim()];
};
